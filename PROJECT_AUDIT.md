...
- `scripts/validate-harvest-run.mjs`：Harvest base SHA、整文件 hash、draft 和 diff 完整性门禁。
...
- 后续应补 OG 图片和结构化 `HowTo`/`TechArticle` 数据。

## 11. 自动化与 Harvest 兼容性

当前仓库仍没有 Daily Harvest/Weekly Intelligence 的实际生成器；本轮已把此前散落在 prose 的发布安全边界落成机器门禁。自动化分支现在必须提交单一 run manifest，PR workflow 会验证 baseCommit、整文件 base/proposed SHA-256、实际 Cookbook diff、draft 状态、Official 来源、redirect、no-op、重复路径和 traversal/symlink。
...
- 文档明确只开 draft PR，自动化不得直写 main；远端 `Protect main` ruleset 第一阶段现已启用，要求 PR 并禁止删除/force push，空 bypass。
- 期望策略已版本化到 `.github/rulesets/main.json`，只读 checker 会核对 PR、force-push/deletion、空 bypass、精确 check context、workflow 可用性和参数漂移。
- 2026-07-13 初始远端审计确认 rulesets 为空；15:40 +08:00 已创建 Active `Protect main`（ID `18863035`）。Content quality workflow 仍未进入默认分支，故 required check 尚未安全启用，checker 继续按设计报告这两项精确 drift。

仍存在的自动化风险：

- 已有 runId、base commit 和整文件 hash；尚无 managed-fields 三方合并，一期选择更严格的整文件冲突失败。
- 同一天日志追加顺序混乱、URL 大量重复，容易并发冲突和产生无意义 diff。
- ruleset 当前没有 bypass，但 `Validate pull request` 尚未成为必需检查；PR 已是硬边界，内容质量 CI 仍需完成第二阶段接线。
- 索引和正文元信息人工重复维护，已经漂移。

下一阶段流程：

1. 每次运行继续写 reviewable run manifest；如需来源抓取明细，再增加不可变 `sources/runs/<timestamp>-<runId>.json`。
2. 以 normalized URL + canonicalArticleId + evidenceHash 为幂等键。
3. 优先用 slug/id、aliases、symptoms 和相似度匹配已有 canonical。
4. 基于 baseCommit/contentHash 生成 patch；发现人工修改即停止或三方合并。
5. 依次运行 schema、发布状态、来源、Official claim、Experimental 风险、内部链接、重复候选和 no-op diff 校验。
6. 保持 draft PR；人工复核后再 reviewed/canonical，并在 GitHub 启用必需检查和禁止 bypass。
...
| P0 | Harvest 自动 canonical/main、缺冲突停止 | 远端 ruleset 第一阶段已启用：PR、删除/force-push 防护、空 bypass；required check 待 workflow 合并并成功运行后补齐 |
...
1. 通过当前已强制的 PR 流程发布 Content quality workflow 及完整依赖闭包；该 PR 的 `Validate pull request` 成功后，先更新现有 ruleset ID `18863035` 加入精确 context，再合并。继续保持空 bypass，并让真实 Harvest 采用 manifest/draft PR 契约。
2. 在 ECS 生产数据目录实际运行 doctor、加密快照和离线 `verify`，异地保存并做恢复前演练，再设计 SQLite/外部数据库双写迁移；当前文件锁是单机安全边界，不是最终事务存储。
3. 剩余 32 篇 v1 迁移按当前用户优先级暂停；完成远端治理第二阶段后再恢复逐篇处理，不做批量迁移。
