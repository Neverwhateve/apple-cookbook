...
| P0 | Harvest 自动 canonical/main、缺冲突停止 | ruleset、strict required check、workflow、删除/force-push 防护与空 bypass 均已生效；真实生成器接入待处理 |
...
1. 让真实 Harvest 采用已生效的 manifest/draft PR 契约，先执行一次无破坏试运行并确认不会直写 `main`、不会覆盖人工修改、无实质变化时不产生 diff。
